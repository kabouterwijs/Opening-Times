namespace :db do
  namespace :mysql do
    ...
    task :public_export => :environment do
      current_date = Time.now.strftime("%Y_%m_%d")
      archive = "#{RAILS_ROOT}/public/data/public_export.sql"
      database, user, password = retrieve_db_info

      cmd = "/usr/bin/env mysqldump --opt --skip-add-locks --ignore-table=#{database}.users -u#{user} "
      puts cmd + "... [password filtered]"
      cmd += " -p'#{password}' " unless password.nil?
      cmd += " #{database} > #{archive}"
      result = system(cmd)
      system("gzip -f -9 #{archive}")
    end
